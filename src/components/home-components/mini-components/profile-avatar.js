...
const ProfileAvatar = (props) => {
    ...
    useEffect(() => {
        const onClick = (e) => {
            if ((reference.current !== null && !reference.current.contains(e.target)) ||
                (e.target === linkDropDown3.current || e.target === linkDropDown2.current || e.target === linkDropDown1.current)) {
                closeDropDown();
            }
    ...
    const url = 'https://cdn1.vectorstock.com/i/1000x1000/51/05/male-profile-avatar-with-brown-hair-vector-12055105.jpg'
    ...
    return (
        <Wrapper ref={reference} onClick={showDropDown} className={'dropdown-menu'}>
            ...
            <div onClick={showDropDown} style={{position: 'relative'}}>
                <UserName transparent={props.transparent}>{'tung, ' + user.userData.firstName}</UserName>
            </div>

            <DropDownDiv dropdown={dropDown} transparent={props.transparent}>
                <Ul transparent={props.transparent}>
                    <li><Link ref={linkDropDown1} to={'/dashboard/page'}> Aktivitet e profilit</Link></li>
                    <li><Link ref={linkDropDown2} to={'/#mes'}> Mesazhet</Link></li>
                    <li><Link ref={linkDropDown3} to={'/#pro'} onClick={AuthService.logout}>Ckycu </Link></li>
                </Ul>
                <GoTriangleUp style={{position: 'absolute', right: '10px', top: '-13px'}}
                              color={props.transparent ? '#303030' : 'white'} size={20}/>
            </DropDownDiv>
